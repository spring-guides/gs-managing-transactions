...
If you use Maven, visit the https://start.spring.io/#!type=maven-project&language=java&platformVersion=2.4.3.RELEASE&packaging=jar&jvmVersion=1.8&groupId=com.example&artifactId=managing-transactions&name=managing-transactions&description=Demo%20project%20for%20Spring%20Boot&packageName=com.example.managing-transactions&dependencies=data-jdbc,h2[Spring Initializr] to generate a new project with the required dependencies (Spring Data JDBC and H2 Database).
