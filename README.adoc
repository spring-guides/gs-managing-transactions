...
You can get a Maven build file with the necessary dependencies directly from the https://start.spring.io/#!type=maven-project&language=java&platformVersion=2.4.2.RELEASE&packaging=jar&jvmVersion=11&groupId=com.example&artifactId=managing-transactions&name=managing-transactions&description=Demo%20project%20for%20Spring%20Boot&packageName=com.example.managing-transactions&dependencies=data-jdbc,h2[Spring Initializr].
